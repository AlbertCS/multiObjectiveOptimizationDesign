...
from mood.metrics.rosetta_extras import movers as rs_movers
from mood.metrics.rosetta_extras import rosettaScripts as rs
from mood.metrics.rosetta_extras import scorefunctions as rs_scorefunctions
...
def abinitio_perturbers(loop_residues, loop_remodel=False):
    ...
    p_set_dihedrals = rs_movers.generalizedKic.pertubers.setDihedrals(
        residues1, atoms1, residues2, atoms2, 180.0
    )
    p_randomize_by_rama = (
        rs_movers.generalizedKic.pertubers.randomizeBackboneByRamaPrepro(loop_residues)
    )
    ...
    close_bond = rs_movers.generalizedKic.closeBond(
        atom1="C", atom2="N", res1=loose_residue, res2=loose_residue + 1
    )
...
def montecarlo_perturbers(loop_residues):
    ...
    p_perturb_dihedrals = rs_movers.generalizedKic.pertubers.perturbDihedrals(
        residues, atoms1, residues, atoms2, 15.0
    )
...
def add_missing_residues(insertion_point, sequence):
    ...
    addBond = rs_movers.declareBond(
        atom1="C", atom2="N", res1=prepend_point + 1, res2=prepend_point + 2
    )
...
def loopRebuild(
    ...
    for i in range(hanging_residues):
        loop_residues = (
            [min(loop_residues) - 1] + loop_residues + [max(loop_residues) + 1]
        )
    ...
    rs_not_neighbors = rs.residueSelectors.notSelector(
        "notNeighbors", rs_neighbors.name
    )
    rs_loop_and_neighbors = rs.residueSelectors.orSelector(
        "loopAndNeighbors", [rs_loop.name, rs_neighbors.name]
    )
    rs_not_loop_and_neighbors = rs.residueSelectors.andSelector(
        "notLoopAndNeighbors", [rs_not_loop.name, rs_not_neighbors.name]
    )
    ...
    to_repack_loop_neighbors = rs.taskOperations.operateOnResidueSubset(
        "repackLoopAndNeighbors",
        rs_loop_and_neighbors.name,
        operation="RestrictToRepackingRLT",
    )
    to_not_repack_else = rs.taskOperations.operateOnResidueSubset(
        "noRepackElse", rs_not_loop_and_neighbors.name, operation="PreventRepackingRLT"
    )
    ...
    m_genKicAI = rs_movers.generalizedKic(
        "genkicAI",
        selector="lowest_energy_selector",
        selector_scorefunction="bb_only",
        closure_attempts=5000,
        stop_when_n_solutions_found=20,
    )

    m_genKicAI.addKICResidues(loop_residues)  # Add loop residues
    p_set_dihedrals, p_randomize_by_rama, close_bond = abinitio_perturbers(
        loop_residues
    )  # Get abinitio perturbers
    ...
    m_genKicMC = rs_movers.generalizedKic(
        "genkicMC",
        selector="lowest_delta_torsion_selector",
        selector_scorefunction="bb_only",
        stop_when_n_solutions_found=1,
    )
    m_genKicMC.addKICResidues(loop_residues)
    p_perturb_dihedrals = montecarlo_perturbers(
        loop_residues
    )  # Get montecarlo perturber
    ...
    m_packRotamer = rs_movers.packRotamersMover(
        "packRot",
        scorefxn=scorefxn,
        task_operations=[to_repack_loop_neighbors, to_not_repack_else, to_extra_chi],
    )
    # Add packrotamer mover
    xml_script.addMover(m_packRotamer)

    # Define parsed protocol
    m_parsed_protocol = rs_movers.parsedProtocol(
        "mc_moves", movers=[m_genKicMC, m_packRotamer]
    )
    # Add parsed protocol mover
    xml_script.addMover(m_parsed_protocol)

    # Define montecarlo sampling mover
    m_montecarlo = rs_movers.genericMonteCarlo(
        "mc_mover", mover=m_parsed_protocol, trials=mc_trials, scorefxn=scorefxn
    )
