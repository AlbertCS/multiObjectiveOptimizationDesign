...
class xmlScript:
    ...
    def __init__(self):
        ...
        self.residueSelectors["root"] = self.xml.SubElement(
            self.root, "RESIDUE_SELECTORS"
        )
        self.jumpSelectors = {}
        ...
        self.moveMapFactories["root"] = self.xml.SubElement(
            self.root, "MOVE_MAP_FACTORIES"
        )
        self.simpleMetrics = {}
    ...
    def addResidueSelector(self, residue_selector):
        ...
        self.residueSelectors["root"].append(
            self.residueSelectors[residue_selector.name].root
        )
    ...
    def addTaskOperation(self, task_operation):
        ...
        self.taskOperations["root"].append(
            self.taskOperations[task_operation.name].root
        )
    ...
    def addMoveMapFactory(self, movemap_factory):
        ...
        self.moveMapFactories["root"].append(
            self.moveMapFactories[movemap_factory.name].root
        )
    ...
    def setProtocol(self, protocol_list=None):
        if protocol_list == None:
            raise ValueError(
                "A list of movers and filters with the execution order is necessary"
            )

        for mover in protocol_list:
            self.protocols[mover.name] = self.xml.SubElement(
                self.protocols["root"], "Add"
            )
            if mover.type == "mover":
                self.protocols[mover.name].set("mover", mover.name)
            elif mover.type == "filter":
                self.protocols[mover.name].set("filter", mover.name)
            elif mover.type == "metric":
                self.protocols[mover.name].set("metrics", mover.name)
                self.protocols[mover.name].set("labels", mover.label)
            else:
    ...
    def write_xml(self, file_name):
        xmlstr = minidom.parseString(self.xml.tostring(self.root)).toprettyxml(
            indent="  "
        )
...
class taskOperations:
    ...
    class operateOnResidueSubset:
        ...
        def __init__(self, name, selector, operation=None):
            ...
            if operation == None or operation not in [
                "RestrictToRepackingRLT",
                "PreventRepackingRLT",
            ]:
                raise ValueError(
                    "Must define one of these operations: RestrictToRepackingRLT, PreventRepackingRLT"
                )
