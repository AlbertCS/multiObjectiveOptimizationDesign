...
class movers:
    ...
    class insertPoseIntoPoseMover:
        ...
        def __init__(self, name="insertPoseIntoPoseMover", start_res_num=None, end_res_num=None,
                     copy_pdbinfo=False, spm_reference_name=None):
    ...
    class interfaceAnalyzerMover:
        ...
        def __init__(self, name="interfaceAnalyzerMover", scorefxn=None, pack_separated=False,
            ...
            if self.jump:
                self.name = f"{name}_{jump}"
            elif self.fixedchains:
                self.name = f"{name}_{fixedchains}"
            elif self.interface:
                self.name = f"{name}_{interface}"
            elif ligandchain:
                self.name = f"{name}_{ligandchain}"
    ...
    class interfaceScoreCalculator:
        ...
        def generateXml(self):
            ...
            self.root.set('name', self.name)
            if self.chains != None:
                self.root.set('chains', ','.join(self.chains))
            if self.scorefxn != None:
                if isinstance(self.scorefxn, str):
                    self.root.set('scorefxn', self.scorefxn)
                else:
